...
-record(context, {docroot,fullpath,fileinfo,response_body,override_path}).

init(Args) ->
    {ok, App}= application:get_application(),
    PrivDir = code:priv_dir(App),
    Override_path = proplists:get_value(override_path, Args),
    {ok, #context{docroot=PrivDir, override_path=Override_path}}.

allowed_methods(ReqData, Context) ->
    {['HEAD', 'GET'], ReqData, Context}.

% Provide 404s if the object isn't actually there
resource_exists(ReqData, Ctx) ->
    Path = case Ctx#context.override_path of
        undefined -> wrq:path(ReqData);
        Override_path -> Override_path
    end,
    Result = case file_exists(Ctx, ["www/" | Path]) of
        {true, _FPath} ->
            true;
        false ->
            false
